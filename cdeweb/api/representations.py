...
def output_xlsx(data, code, headers):
    ...
    data_view = []
    for compound_num, record in enumerate(data['result']):
        for name in record.get('names', []):
            data_view.append({'compound_id': compound_num, 'name': name})
    df = pd.DataFrame.from_records(data_view)
    df.to_excel(writer, sheet_name='compound_names', index=False)
    data_view = []
    for compound_num, record in enumerate(data['result']):
        for label in record.get('labels', []):
            data_view.append({'compound_id': compound_num, 'label': label})
    df = pd.DataFrame.from_records(data_view)
