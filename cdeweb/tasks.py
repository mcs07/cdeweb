...
def run_cde(job_id):
    ...
    if filepath.endswith('.zip'):
        cde_job.result = []
        cde_job.abbreviations = {'abbreviations': []}
        ...
        for zipname in zf.namelist():
            ...
            with zf.open(zipname) as f:
                try:
                    document = Document.from_file(f, fname=f.name)
                except Exception:
                    continue
            cde_job.result.extend([r.to_primitive() for r in document.records])
            cde_job.abbreviations['abbreviations'].extend(document.abbreviation_definitions)
    else:
        with open(filepath) as f:
